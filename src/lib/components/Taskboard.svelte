<script lang="ts">
  ...
  async function saveTask(e: SubmitEvent) {
    ...
    const assigneeString =
      assigneeTags.length > 0 ? assigneeTags.join(", ") : "Unassigned";
            ...
            >
                ...
                <div class="task-labels">
                  {#each task.labels as label}
                    ...
                    {#if labelData}
                      <span
                        class="task-label"
                        style="background-color: {labelData.color}20; color: {labelData.color}; border-color: {labelData.color}40"
                      >
                        {labelData.label}
                      </span>
                    {/if}
              ...
              <div class="task-meta">
                ...
                {#if task.dueDate}
                  <div
                    class="task-due-date"
                    class:overdue={isOverdue(task.dueDate, task.id)}
                  >
...
{#if showDetailModal && selectedTask}
  ...
  >
    <div
      class="modal-content detail-modal-content"
      role="presentation"
    ...
    >
              ...
              <div>
                ...
                <span
                  class:overdue={isOverdue(
                    ...
                    selectedTask.id,
                  )}>📅 {formatDate(selectedTask.dueDate)}</span
                >
              </div>
            {/if}
            <div>
      ...
      <div class="modal-actions">
        ...
        <button
          ...
          onclick={() => openTaskModal(selectedTask || undefined)}
        >
          Edit
        </button>
      </div>
      ...
      <form bind:this={taskForm} onsubmit={saveTask}>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="date" id="taskDueDate" bind:this={taskDueDate} />
          </div>
        ...
        <div class="form-group">
          ...
          <div class="tag-input-container">
            <div
              class="tag-input-wrapper border border-slate-200 dark:border-slate-700 rounded-xl p-2 min-h-[48px] bg-slate-50 dark:bg-slate-900 flex flex-wrap gap-2 focus-within:border-[#9f86ff] focus-within:ring-2 focus-within:ring-[#9f86ff]/20 transition-all"
            >
              {#each assigneeTags as tag}
                <span
                  class="assignee-tag bg-[#9f86ff]/10 text-[#9f86ff] border border-[#9f86ff]/20 px-3 py-1 rounded-full text-sm font-semibold flex items-center gap-2"
                >
                  {tag}
                  <button
                    type="button"
            ...
            <div class="label-selector">
              {#each LABELS as label}
                <button
                  ...
                  style="background-color: {selectedLabels.includes(label.id)
                    ? label.color + '20'
                    : 'transparent'}; border-color: {label.color}40; color: {label.color}"
